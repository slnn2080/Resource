<template>
  <div class="form-group col-3">
    <label for="ai_analysis_flag">{{ displayLang.EXAMINEES_CONDITIONS_AI_ANALYSIS_FLAG }}</label>
    <select id="ai_analysis_flag" v-model="value" class="form-control">
...
export default Vue.extend({
  ...
  computed: {
    ...
    value: {
      get(): string {
        return this.$store.getters[commonExamineesTypes.GETTER_COMMON_EXAMINEES_GET_AI_ANALYSIS_FLAG];
      },
      set(value: string) {
        this.$store.dispatch(commonExamineesTypes.ACTION_COMMON_EXAMINEES_SET_AI_ANALYSIS_FLAG, value);
      }
    },
  },
});
